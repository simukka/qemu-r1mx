...
DEVICE_INFO: Dict[int, Tuple[str, str, int]] = {
    ...
    DEV_CPU:       ("CPU",       "PPC405F6 CPU",         0x0000_0000),
    DEV_RAM:       ("RAM",       "System RAM",           0x0000_0000),
    DEV_ROM:       ("ROM",       "NOR Flash / Boot ROM", 0xF000_0000),
    DEV_SDCARD:    ("SDCARD",    "SD / CF Card",         0x0000_0000),
    DEV_SSD:       ("SSD",       "SSD (SiI3512 SATA)",   0x0000_0000),
}
...
REG_MAP: Dict[int, Tuple[str, str]] = {
    ...
    # NOR flash / boot ROM (128 MB at 0xF0000000; 64 KB alias at 0xFFFF0000)
    0xF000_0000: ("NOR_BASE",   "NOR flash base (erased = 0xFF)"),
    0xFFFF_0000: ("BOOT_ROM",   "Boot ROM 64 KB alias"),
}


def decode_register(addr: int, dev_id: int = -1) -> str:
    """Return 'REGNAME' if address is in the map, or a decoded summary."""
    # CPU / RAM: addr field holds the current PC
    if dev_id in (DEV_CPU, DEV_RAM):
        return f"PC=0x{addr:08X}"
    # Block devices: addr=0, value carries the KB delta (decoded by caller)
    if dev_id in (DEV_SDCARD, DEV_SSD):
        return "I/O-delta"
    if addr in REG_MAP:
        return REG_MAP[addr][0]
    # Find closest device base for offset display
    for _dev_id, (_, _, base) in DEVICE_INFO.items():
        if base != 0 and base <= addr < base + 0x40000:
...
class ActivityPacket:
    ...
    def __post_init__(self) -> None:
        ...
        self.reg_name  = decode_register(self.addr, self.dev_id)
    ...
    def value_hex(self) -> str:
        if self.dev_id in (DEV_SDCARD, DEV_SSD):
            return f"{self.value} KB"
        if self.dev_id == DEV_CPU:
            return "running" if self.value else "halted"
        width = self.size * 2
...
class DeviceListPanel(QWidget):
    # Emits the dev_id that was clicked, or -1 when selection is cleared.
    device_clicked = pyqtSignal(int)

    _SEL_STYLE   = ("QPushButton { background:#1f3a6e; color:#e6edf3;"
                    " border:1px solid #3a82f7; border-radius:3px;"
                    " font-size:11px; text-align:left; padding:1px 4px; }")
    _NORM_STYLE  = ("QPushButton { background:transparent; color:#e6edf3;"
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._selected_dev: int = -1

        layout = QVBoxLayout(self)
        layout.setSpacing(2)
        layout.setContentsMargins(6, 6, 6, 6)

        # Header row: label + "All" clear button
        hdr_row = QHBoxLayout()
        hdr_lbl = QLabel("DEVICES")
        hdr_lbl.setStyleSheet("font-weight: bold; font-size: 10px; color: #888;")
        hdr_row.addWidget(hdr_lbl)
        hdr_row.addStretch()
        all_btn = QPushButton("All")
        all_btn.setFixedSize(32, 16)
        all_btn.setStyleSheet(
            "QPushButton { font-size:9px; padding:0; background:#21262d;"
            " border:1px solid #30363d; border-radius:2px; color:#8b949e; }"
            "QPushButton:hover { color:#e6edf3; }")
        all_btn.setToolTip("Show all devices (clear device filter)")
        all_btn.clicked.connect(lambda: self._select(-1))
        hdr_row.addWidget(all_btn)
        ...
        for dev_id in sorted(DEVICE_INFO):
            ...
            btn.setToolTip(
                f"{long_name}\nBase: 0x{base:08X}\n"
                "Click to filter log to this device")
            btn.setFixedWidth(80)
            btn.setStyleSheet(self._NORM_STYLE)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.clicked.connect(lambda checked, d=dev_id: self._select(d))
            self._btns[dev_id] = btn
            row.addWidget(btn)
    ...
    def _select(self, dev_id: int) -> None:
        """Toggle selection: clicking the active device deselects (show all)."""
        if dev_id == self._selected_dev:
            dev_id = -1   # deselect
        prev = self._selected_dev
        self._selected_dev = dev_id
        # Update button styles
        if prev in self._btns:
            self._btns[prev].setStyleSheet(self._NORM_STYLE)
        if dev_id in self._btns:
            self._btns[dev_id].setStyleSheet(self._SEL_STYLE)
        self.device_clicked.emit(dev_id)

    def selected_dev(self) -> int:
...
_LOG_MAX     = None          # unlimited — keep every event for RE analysis
_COL_TS   = 0
_COL_DEV  = 1
_COL_DIR  = 2
_COL_ADDR = 3
...
class EventLogModel(QAbstractTableModel):
    def __init__(self, parent: Optional[QObject] = None) -> None:
        super().__init__(parent)
        self._rows: Deque[ActivityPacket] = deque()   # unlimited
        self._filtered: List[ActivityPacket] = []     # active view
        self._filter_text: str = ""
        self._filter_dev:  int  = -1   # -1 = all devices
    ...
    def set_device_filter(self, dev_id: int) -> None:
        """Show only packets from dev_id, or all if dev_id == -1."""
        self._filter_dev = dev_id
        self._rebuild_filtered()

    def _match(self, p: ActivityPacket) -> bool:
        if self._filter_dev != -1 and p.dev_id != self._filter_dev:
            return False
        if self._filter_text:
            ft = self._filter_text
            return (
                ft in p.dev_short.lower()
                or ft in p.reg_name.lower()
                or ft in p.direction.lower()
                or ft in f"0x{p.addr:08x}"
                or ft in p.value_hex.lower()
            )
        return True

    def _rebuild_filtered(self) -> None:
        self.beginResetModel()
        self._filtered = [p for p in self._rows if self._match(p)]
        self.endResetModel()

    # --- bulk append (reduces insertRows signal overhead) ---

    def append_batch(self, pkts: List[ActivityPacket]) -> None:
        if not pkts:
            return
        for p in pkts:
            self._rows.append(p)
        new_visible = [p for p in pkts if self._match(p)]
        if new_visible:
            first = len(self._filtered)
            last  = first + len(new_visible) - 1
            self.beginInsertRows(QModelIndex(), first, last)
            self._filtered.extend(new_visible)
            self.endInsertRows()

    def append(self, pkt: ActivityPacket) -> None:
        self.append_batch([pkt])
    ...
    def all_rows(self) -> Deque[ActivityPacket]:
        """Return the full unfiltered packet deque (for export)."""
        return self._rows
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._dev_panel.device_clicked.connect(self._on_device_clicked)
        ...
        filter_hdr = QLabel("EVENT LOG")
        filter_hdr.setStyleSheet(
            "font-weight: bold; font-size: 10px; color: #888; padding: 2px 4px;")
        filter_row.addWidget(filter_hdr)
        from PyQt6.QtWidgets import QLineEdit
        self._filter_edit = QLineEdit()
        self._filter_edit.setPlaceholderText("Filter: device / reg / addr / value…")
        ...
        # Batch-flush timer — coalesce rapid packets before updating model
        self._pending: List[ActivityPacket] = []
        self._flush_timer = QTimer(self)
        self._flush_timer.setInterval(40)   # flush to model every 40 ms
        self._flush_timer.timeout.connect(self._flush_pending)
        self._flush_timer.start()

        # CSV auto-log
        self._csv_path   = Path.home() / "r1mx_activity.csv"
        self._csv_file   = open(self._csv_path, "w", newline="", buffering=1)
        self._csv_writer = csv.writer(self._csv_file)
        self._csv_writer.writerow(
            ["ts_ns", "ts_ms", "dev_id", "device", "dir",
             "addr_hex", "register", "value_hex", "size"])
        self._status_bar.showMessage(
            f"Logging to {self._csv_path}", 0)
    ...
    def _on_filter_changed(self, text: str) -> None:
        self._log_model.set_filter(text)

    def _on_device_clicked(self, dev_id: int) -> None:
        self._log_model.set_device_filter(dev_id)
        # Update the filter box placeholder to reflect the active device filter
        if dev_id == -1:
            self._filter_edit.setPlaceholderText(
                "Filter: device / reg / addr / value…")
        else:
            short = DEVICE_INFO.get(dev_id, (str(dev_id), "", 0))[0]
            self._filter_edit.setPlaceholderText(
                f"Showing {short} — type to narrow further…")
    ...
    def _flush_pending(self) -> None:
        ...
        batch = self._pending
        self._pending = []
        self._log_model.append_batch(batch)
        self._event_count += len(batch)
        self._events_label.setText(
            f"Events: {self._event_count:,}  "
            f"(filtered: {self._log_model.rowCount():,})")

    def _export_csv(self) -> None:
        from PyQt6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getSaveFileName(
            self, "Export event log",
            str(Path.home() / "r1mx_export.csv"),
            "CSV files (*.csv)")
        if not path:
            return
        with open(path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["ts_ns", "ts_ms", "dev_id", "device", "dir",
                        "addr_hex", "register", "value_hex", "size"])
            for pkt in self._log_model.all_rows():
                w.writerow([
                    pkt.ts_ns, pkt.ts_ms_str, pkt.dev_id, pkt.dev_long,
                    pkt.direction, f"0x{pkt.addr:08X}",
                    pkt.reg_name, pkt.value_hex, pkt.size,
                ])
        self._status_bar.showMessage(f"Exported {self._event_count:,} events → {path}", 5000)
    ...
    def _on_packet(self, pkt: ActivityPacket) -> None:
        # Write to CSV immediately (line-buffered)
        self._csv_writer.writerow([
            pkt.ts_ns, pkt.ts_ms_str, pkt.dev_id, pkt.dev_long,
            pkt.direction, f"0x{pkt.addr:08X}",
            pkt.reg_name, pkt.value_hex, pkt.size,
        ])
        # Enqueue for batched model update
